...
public class DomainLevelExceptionsHandler {
    ...
    @ExceptionHandler(OriginAlredyLikesTargetException.class)
    @ResponseBody
    public ProblemDetail handleOriginAlreadyLikesDestinationException(OriginAlredyLikesTargetException ex) {
        ProblemDetail pd = ProblemDetail.forStatusAndDetail(HttpStatus.CONFLICT, ex.getMessage());
        pd.setTitle("Duplicated like");
        pd.setProperty("timestamp", Instant.now());
        return pd;
    }
}
